...
export const fetchStores = createAsyncThunk('stores/fetchStores', async () => {
    const response = await axios.get('https://localhost:7113/api/stores');
    return response.data;
});

export const createStore = createAsyncThunk('stores/createStore', async (newStore) => {
    const response = await axios.post('https://localhost:7113/api/stores', newStore);
    return response.data;
});

export const updateStore = createAsyncThunk('stores/updateStore', async (updatedStore) => {
    const response = await axios.put(`https://localhost:7113/api/stores/${updatedStore.id}`, updatedStore);
    return response.data;
});

export const deleteStore = createAsyncThunk('stores/deleteStore', async (storeId) => {
    await axios.delete(`https://localhost:7113/api/stores/${storeId}`);
    return storeId;
});
